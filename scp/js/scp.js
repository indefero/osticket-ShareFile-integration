 ...
 */
 ...
 //SHARE FILE INTEGRATION FUNCTIONALITY
...
//official polyfill for string.includes (not included in older IE versions)
...
//END SHARE FILE INTEGRATION FUNCTIONALITY
...
var scp_prep = function() {
	//SHARE FILE INTEGRATION FUNCTIONALITY
	$('#SF_INTEGRATION_GEN_LINK').click(function(e) {
        e.preventDefault();
        var hostNameKit = "kit";
        var hostNameOSTD = "ostd";
        var hostName = window.location.hostname;
        if(hostName.includes(hostNameKit)){
            hostName = hostNameKit;
        }
        else if(hostName.includes(hostNameOSTD)){
            hostName = hostNameOSTD;
        }
		$.post('http://'+ hostName +'/osticket/include/shareFile/generateSFLink.php', 'id=' + getQueryVariable("id"), function (response) {
			//update text box
			//var fObj = $('#SF_INTEGRATION_GEN_LINK').closest('form');
			//var box = $('#response',fObj);
			//var redactor = box.data('redactor');
            var box = $('#response');
			var redactor = box.data('redactor');
            if(response) {
                if (redactor){
                    redactor.insert.html(response);
                }
                else{
    ...
    $('#SF_INTEGRATION_GEN_LINK_TAG').click(function(e) {
        e.preventDefault();
        var box = $('#response');
        var tag = "<b>__SF_TAG_LINK__</b>"
        var redactor = box.data('redactor');
        if (redactor){
            redactor.insert.html(tag);
        }
        else{
            box.val(box.val() + tag);
        }
