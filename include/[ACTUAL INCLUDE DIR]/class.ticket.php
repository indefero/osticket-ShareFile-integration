...
implements RestrictedAccess, Threadable {
    ...
    function onNewTicket($message, $autorespond=true, $alertstaff=true) {
        ...
        if ($alertstaff
            ...
            if ($cfg->alertDeptMembersONNewTicket() && !$this->isAssigned()
                && ($members = $dept->getMembersForAlerts()) //removed ->all to stop all members from getting alerts
            ) {
    ...
    function postMessage($vars, $origin='', $alerts=true) {
        ...
        if ($cfg->alertONNewMessage()
            ...
            if ($cfg->alertAssignedONNewMessage() && $this->isAssigned()) {
                if ($staff = $this->getStaff())
                    $recipients[] = $staff;
             //commented out to disable team alerts on ticket updates
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    function getStaffStats($staff) {
        ...
        // -- Open and assigned to a team of mine (11/21/2016/ - Commented out to fix My Tickets tab count - DGM)
    ...
    static function open($vars, &$errors) {
        ...
        if($vars['response'] && $role->hasPerm(TicketModel::PERM_REPLY)) {
            ...
            //BEGIN SHARE FILE ADDON
            require_once(INCLUDE_DIR.'shareFile/generateSFLink.php');
            require_once(INCLUDE_DIR.'shareFile/ErrorLog.php');
            $containsShareFileTag = false;
            if(strpos($vars['response'], '__SF_TAG_LINK__') !== false ){
                $containsShareFileTag = true;
            }
            $vars['response'] = preg_replace('/__SF_TAG_LINK__/',  \shareFile\GenerateSFLinkFromTicketID($ticket->getId()), $vars['response']);
            \shareFile\C_SF_ErrorLog::WriteError("Link Replaced", \shareFile\C_SF_ErrorLog::DEBUG);
            //END SHARE FILE ADDON
            ...
            //Edited for shareFile integration
            $response = $ticket->postReply($vars, $errors, $containsShareFileTag);
        }
